...
- **부 아이콘**: `book.yaml`의 `parts[].icon`에 astro-icon 이름(`lucide:cpu`)을 적으면
  ...
  안 적은 책은 아이콘 없이 나간다
- **장의 첫 문단을 키우지 말 것** (2026-09-20에 빼고 그 이유를 `book.css`에 적어 뒀다).
  원래 `.book-chapter-body > p:first-child`에 `1.075em`이 걸려 있었는데 본문 18px과 1.4px
  차이라 아무도 못 알아봤고, `1.2em`(21.6px)으로 올리니 저자가 "뭔가 이상하다"고 했다.
  **도입부는 글이 표시하는 것이지 글자 크기가 표시하는 것이 아니다** — 장을 여는 문단은
  사람과 장면으로 시작하므로 그것으로 이미 구별된다. 다시 넣고 싶어지면 크기 말고 여백부터 본다.
  덧붙여 그 선택자는 **1장에 아예 안 걸리고 있었다.** 1장의 첫 요소가 면책 인용문(`>`)이라
  `p:first-child`가 매치되지 않는다. 첫 요소가 그림·표·인용문인 장이 있다는 것을 잊지 말 것
