...
각 단계를 따로 보면 참 자연스럽다. 핫픽스 워크플로우가 원래 이런 식으로 흘러가고, 대개 이렇게들 생각한다.
...
문제는 같은 feature 브랜치를 develop에 다시 머지해도 코드가 안 들어온다는 데 있다. Git 입장에서는 이미 머지했던 commit이라 그냥 건너뛰기 때문이다. 모르고 지나가면 feature가 영영 develop에서 사라진 채로 배포된다. 필자도 이거 때문에 엄청난 시간을 날렸다.
...
메인에 적용한 hotfix가 revert됐을 때 무심코 지나가는 경우가 있다.

### 잘못된 접근 1: develop 코드 방치
...
- PR 한 번으로 끝
- history가 정직하다. "실수로 revert했다가 되살림" 흐름이 그대로 남는다
- develop을 안 건드려도 된다. 코드가 살아있다면 그대로 두고 main만 고치면 자동으로 정상화된다
...
cherry-pick은 **diff를 새 SHA로 재적용**하기 때문에 Git 입장에서는 별개의 commit이다. 이 새 commit이 양쪽 ancestry에 들어가면 꼬인 게 풀린다.
...
지금까지는 revert가 의도된 것이었다면 양쪽에 대칭으로 적용하라는 얘기였다. 그러면 **revert 자체가 실수**였을 때는 어떻게 해야 할까.

이럴 때는 develop의 코드를 그냥 놔두고 싶어지지만, main의 revert가 미래 머지의 base가 되어 결국 그 코드를 지워버린다.

답은 여기서도 **main에서 revert the revert**다. 위의 두 방법 중 하나를 골라 적용하면 된다.
